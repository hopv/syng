...
module Syng.Logic.Example where
...
open import Syng.Lang.Expr using (Addr; ◸_; _↷_; Expr∞; Expr˂∞; ∇_; 🞰_; Type;
  TyVal; _←_; cas; loop)
open import Syng.Lang.Ktxred using (_←ᴿ_; casᴿ)
open import Syng.Lang.Example using (plus◁3,4; decrep; decrep'; ndecrep; evrep;
  fadᴿ; fad; fadrep; fadrep'; xfadrep; nxfadrep; cntr←)
open import Syng.Logic.Prop using (Name; strnm; SProp; SProp˂; SProp∞; SProp˂∞;
  ¡ᴾ_; ∀-syntax; ∃-syntax; _∨_; ⊤'; ⊥'; ⌜_⌝∧_; ⌜_⌝; _∗_; □_; ○_; _↦_; _↦⟨_⟩_;
  _⊸[_]⇛_; _⊸⟨_⟩ᵀ[_]_; [^_]ᴺ; &ⁱ⟨_⟩_; static; _↦ⁱ_; #ᵁᵇ⟨_⟩_; ≤ᵁᵇ⟨_⟩_; ^ᶻᴺ-✔)
open import Syng.Logic.Core using (_⊢[_]_; Pers; ⊢-refl; _»_; ∀-intro; ∃-elim;
  ...
  dup-Pers-∗)
open import Syng.Logic.Fupd using (_⊢[_][_]⇛_; ⤇⇒⇛; ⇒⇛; _ᵘ»ᵘ_; _ᵘ»_; ⇛-frameˡ;
  ⇛-frameʳ)
open import Syng.Logic.Hor using (_⊢[_][_]ᵃ⟨_⟩_; _⊢[_]⟨_⟩ᴾ_; _⊢[_]⟨_⟩ᵀ[_]_;
...
open import Syng.Logic.Heap using (↦⟨⟩-merge-/2; ↦⟨⟩-split-/2; ↦⟨⟩-agree;
  ahor-←; ahor-cas-tt; ahor-cas-ff; ahor-fau; hor-🞰; hor-←)
open import Syng.Logic.Ind using (○-mono; ○-new; □○-new-Pers; □○-new-rec-Pers;
  ○-use; ○⇒⊸⇛; ⊸⇛-use; ○⇒⊸⟨⟩; ⊸⟨⟩ᵀ-use)
open import Syng.Logic.Inv using (&ⁱ-new; &ⁱ-open; ⅋ⁱ-close; ahor-&ⁱ-use;
  hor-↦ⁱ-🞰)
open import Syng.Logic.Ub using (≤ᵁᵇ-#ᵁᵇ; #ᵁᵇ-new; #ᵁᵇ-upd)
