...
module Symp.Model.Fupd.Base where
...
open import Symp.Model.ERA.Glob using (iᴹᵉᵐ; outᴳ; Envᴵⁿᴳ; Envᴵⁿᴳ˙; envᴳ;
...
⟨_⟩[_]⇛ᴳ'⟨_⟩_ :  ∀{X : Set ł} →
  Mem →  (Envᴵⁿᴳ → X) × (X → Envᴵⁿᴳ → Envᴵⁿᴳ) × (X → SPropᵒ ł') →
  Mem →  SPropᵒ ł'' →  SPropᵒ (1ᴸ ⊔ᴸ ł ⊔ᴸ ł' ⊔ᴸ ł'')
