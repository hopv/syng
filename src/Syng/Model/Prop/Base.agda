...
module Syng.Model.Prop.Base where
...
open import Syng.Model.ERA.Glob using (Globᴱᴿᴬ; Globᴱᴿᴬ˙; Resᴳ; Resᴳ˙; Envᴳ;
...
--------------------------------------------------------------------------------
-- SPropᵒ :  Semantic separation-logic proposition

SPropᵒ :  ∀ ł →  Set (1ᴸ ⊔ᴸ ṡᴸ ł)
SPropᵒ ł =  Resᴳ →  Set ł

-- Monoᵒ Pᵒ :  Pᵒ is monotone over the resource

Monoᵒ :  SPropᵒ ł →  Set (1ᴸ ⊔ᴸ ł)
Monoᵒ Pᵒ =  ∀{a b} →  a ⊑ b →  Pᵒ a →  Pᵒ b

private variable
  X Y :  Set ł
  Pᵒ P'ᵒ Qᵒ Q'ᵒ Rᵒ R'ᵒ Sᵒ S'ᵒ Tᵒ T'ᵒ :  SPropᵒ ł
  Pᵒ˙ Qᵒ˙ :  X →  SPropᵒ ł
  Pᵒs :  List (SPropᵒ ł)
  a b :  Resᴳ
  b˙ :  X → Resᴳ
  E F G :  Envᴳ
  E˙ F˙ :  X →  Envᴳ
  FPᵒ˙ GPᵒ˙ FQᵒ˙ :  X →  Envᴳ × SPropᵒ ł
  GPᵒ˙˙ :  X →  Y →  Envᴳ × SPropᵒ ł
  f :  Y → X
  x y :  X
  xs ys :  List X
  ι :  𝕊
...
_⊨✓_ _⊨_ :  SPropᵒ ł →  SPropᵒ ł' →  Set (1ᴸ ⊔ᴸ ł ⊔ᴸ ł')
Pᵒ ⊨✓ Qᵒ =  ∀{E a} →  E ✓ a →  Pᵒ a →  Qᵒ a
Pᵒ ⊨ Qᵒ =  ∀{a} →  Pᵒ a →  Qᵒ a

-- ⊨ Pᵒ :  Tautology

⊨_ :  SPropᵒ ł →  Set (1ᴸ ⊔ᴸ ł)
⊨ Pᵒ =  ∀{a} →  Pᵒ a
...
abstract
  ...
  substᵒ :  ∀(Pᵒ˙ : X → SPropᵒ ł) →  x ≡ y →  Pᵒ˙ x ⊨ Pᵒ˙ y
  substᵒ Pᵒ˙ refl =  id
...
∀ᵒ˙ ∃ᵒ˙ ∃ᴵ˙ ∀ᵒ∈-syntax ∃ᵒ∈-syntax ∃ᴵ∈-syntax ∀ᵒ-syntax ∃ᵒ-syntax ∃ᴵ-syntax :
  ∀{X : Set ł} →  (X → SPropᵒ ł') →  SPropᵒ (ł ⊔ᴸ ł')
∀ᵒ˙ Pᵒ˙ a =  ∀ x →  Pᵒ˙ x a
∃ᵒ˙ Pᵒ˙ a =  ∑ x ,  Pᵒ˙ x a
∃ᴵ˙ Pᵒ˙ a =  ∑ᴵ x ,  Pᵒ˙ x a
∀ᵒ∈-syntax =  ∀ᵒ˙
...
infix 3 ⌜_⌝ᵒ×_ ⌜_⌝ᵒ→_
⌜_⌝ᵒ×_ ⌜_⌝ᵒ→_ :  Set ł →  SPropᵒ ł' →  SPropᵒ (ł ⊔ᴸ ł')
⌜ X ⌝ᵒ× Pᵒ =  ∃ᵒ _ ∈ X , Pᵒ
⌜ X ⌝ᵒ→ Pᵒ =  ∀ᵒ _ ∈ X , Pᵒ
...
infixr 7 _×ᵒ_
_×ᵒ_ :  SPropᵒ ł →  SPropᵒ ł' →  SPropᵒ (ł ⊔ᴸ ł')
(Pᵒ ×ᵒ Qᵒ) a =  Pᵒ a × Qᵒ a
...
abstract
  ...
  infixr 3 _→ᵒ_
  _→ᵒ_ :  SPropᵒ ł →  SPropᵒ ł' →  SPropᵒ (1ᴸ ⊔ᴸ ł ⊔ᴸ ł')
...
_∗ᵒ'_ :  SPropᵒ ł →  SPropᵒ ł' →  SPropᵒ (1ᴸ ⊔ᴸ ł ⊔ᴸ ł')
...
[∗ᵒ∈] [∗ᵒ∈]-syntax :  (X → SPropᵒ ł) →  List X →  SPropᵒ (1ᴸ ⊔ᴸ ł)
[∗ᵒ∈] Pᵒ˙ xs =  [∗ᵒ] $ Pᵒ˙ $ᴸ xs
[∗ᵒ∈]-syntax =  [∗ᵒ∈]
[∗ᵒ∈ⁱ] [∗ᵒ∈ⁱ]-syntax :  (ℕ × X → SPropᵒ ł) →  List X →  SPropᵒ (1ᴸ ⊔ᴸ ł)
[∗ᵒ∈ⁱ] Pᵒ˙ xs =  [∗ᵒ] $ curry Pᵒ˙ $ⁱᴸ xs
[∗ᵒ∈ⁱ]-syntax =  [∗ᵒ∈ⁱ]
[∗ᵒ∈ⁱ⟨⟩] [∗ᵒ∈ⁱ⟨⟩]-syntax :
  (ℕ × X → SPropᵒ ł) →  ℕ →  List X →  SPropᵒ (1ᴸ ⊔ᴸ ł)
...
infix 8 [∗ᵒ∈²]-syntax
[∗ᵒ∈²] [∗ᵒ∈²]-syntax :
  (X × Y →  SPropᵒ ł) →  List X →  List Y →  SPropᵒ (1ᴸ ⊔ᴸ ł)
...
_⤇ᴱ'_ :  ∀{X : Set ł'} →
  Envᴳ →  (X → Envᴳ × SPropᵒ ł) →  SPropᵒ (1ᴸ ⊔ᴸ ł ⊔ᴸ ł')
...
infix 3 ⤇ᴱ⟨⟩
⤇ᴱ⟨⟩ :  ∀{X : Set ł'} →  ∀ i →
  Envᴳ˙ i →  (X → Envᴳ˙ i × SPropᵒ ł) →  SPropᵒ (1ᴸ ⊔ᴸ ł ⊔ᴸ ł')
⤇ᴱ⟨⟩ i Eⁱ FⁱPᵒ˙ =  ∀ᵒ G ,
...
Thunkᵒ Shrunkᵒ :  (𝕊 → SPropᵒ ł) →  𝕊 →  SPropᵒ ł
