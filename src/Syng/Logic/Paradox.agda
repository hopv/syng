...
module Syng.Logic.Paradox where

open import Base.Func using (_$_; flip)
open import Base.Eq using (refl)
open import Base.Size using (𝕊; ∞; !)
open import Base.Prod using (-,_)
open import Syng.Lang.Expr using (Type; Expr∞; loop; Val)
open import Syng.Lang.Ktxred using (Redex)
open import Syng.Lang.Reduce using (_⇒ᴾ_; redᴾ)
open import Syng.Logic.Prop using (Name; strnm; Lft; SProp; SProp˂; SProp∞;
  SProp˂∞; ¡ᴾ_; ∃-syntax; _∧_; _∨_; ⊤'; ⊥'; □_; _∗_; _-∗_; [^_]ᴺ; ○_; _⊸[_]⇛_;
  _⊸[_]ᵃ⟨_⟩_; _⊸⟨_⟩ᴾ_; _⊸⟨_⟩ᵀ[_]_; _⊸[_]⟨_⟩∞; &ⁱ⟨_⟩_; ⅋ⁱ⟨_⟩_; [_]ᴸ; †ᴸ_)
open import Syng.Logic.Core using (_⊢[_]_; ⇒<; ⊢-refl; _»_; ∃-elim; ∃-intro;
...
open import Syng.Logic.Ind using (○-mono; □○-new-rec; ○-use; ○⇒⊸⇛; ○⇒⊸ᵃ⟨⟩;
  ○⇒⊸⟨⟩; ○⇒⊸⟨⟩∞)
open import Syng.Logic.Inv using (&ⁱ-new; &ⁱ-open; ⅋ⁱ-close)
open import Syng.Logic.Lft using ([]ᴸ⟨⟩-†ᴸ-no; []ᴸ-new; []ᴸ-kill)
