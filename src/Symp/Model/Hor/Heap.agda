...
open import Symp.Model.Prop.Heap using (_↦⟨_⟩ᵒ_; _↦ᵒ_; Freeᵒ'; Freeᵒ; _↦ᴸᵒ_;
  ...
  ↦ᴸᵒ'-free')
open import Symp.Model.Fupd.Interp using (⟨_⟩⇛ᴴ⟨_⟩_; ?⊨⤇ᴱᴴᵉᵃᵖ⇒?⊨⇛ᴴ; ⊨⤇ᴱᴴᵉᵃᵖ⇒⊨⇛ᴴ;
  ⇛ᴴ-mono; ⇛ᴴ-intro; ⇛ᴴ-intro-✓ᴴ; ⇛ᴴ-eatˡ)
...
abstract
  ...
  ↦⟨⟩ᵒ-read :  θ ↦⟨ p ⟩ᵒ ᵗv  ⊨ ⟨ H ⟩⇛ᴴ⟨ H ⟩
                 ⌜ H ‼ᴴ θ ≡ š ᵗv ⌝ᵒ×  θ ↦⟨ p ⟩ᵒ ᵗv
  ↦⟨⟩ᵒ-read =  ?⊨⤇ᴱᴴᵉᵃᵖ⇒?⊨⇛ᴴ ↦⟨⟩ᵒ-read'

  -- Write using ↦ᵒ

  ↦ᵒ-write :  θ ↦ᵒ ᵗu  ⊨ ⟨ H ⟩⇛ᴴ⟨ updᴴ θ ᵗv H ⟩  θ ↦ᵒ ᵗv
  ↦ᵒ-write {ᵗu = ᵗu} =  ?⊨⤇ᴱᴴᵉᵃᵖ⇒?⊨⇛ᴴ $ ↦ᵒ-write' {ᵗu = ᵗu}

  -- Allocate to get ↦ᴸᵒ' and Freeᵒ'

  ↦ᴸᵒ'-alloc :  H o ≡ ň  →
    ⊨ ⟨ H ⟩⇛ᴴ⟨ upd˙ o (š rep n ⊤-) H ⟩  o ↦ᴸᵒ' rep n ⊤-  ∗ᵒ  Freeᵒ' n o
  ↦ᴸᵒ'-alloc Ho≡ň =  ⊨⤇ᴱᴴᵉᵃᵖ⇒⊨⇛ᴴ $ ↦ᴸᵒ'-alloc' Ho≡ň

  -- Bounds check using Freeᵒ'

  Freeᵒ'-š :  Freeᵒ' n o  ⊨ ⟨ H ⟩⇛ᴴ⟨ H ⟩  ⌜ ∑ ᵗvs , H o ≡ š ᵗvs ⌝ᵒ×  Freeᵒ' n o
  Freeᵒ'-š =  ?⊨⤇ᴱᴴᵉᵃᵖ⇒?⊨⇛ᴴ Freeᵒ'-š'

  -- Free using ↦ᴸᵒ' and Freeᵒ'

  ↦ᴸᵒ'-free :  len ᵗvs ≡ n  →
    o ↦ᴸᵒ' ᵗvs  ∗ᵒ  Freeᵒ' n o  ⊨ ⟨ H ⟩⇛ᴴ⟨ upd˙ o ň H ⟩  ⊤ᵒ₀
  ↦ᴸᵒ'-free lenvs≡n =  ?⊨⤇ᴱᴴᵉᵃᵖ⇒?⊨⇛ᴴ $ ↦ᴸᵒ'-free' lenvs≡n
...
abstract
  ...
  ᵃ⟨⟩ᵒ-🞰 :  θ ↦⟨ p ⟩ᵒ (T , v)  ⊨ ᵃ⟨ 🞰ᴿ_ {T} θ ⟩ᵒ λ u →
              ⌜ u ≡ v ⌝ᵒ×  θ ↦⟨ p ⟩ᵒ (T , v)
  ᵃ⟨⟩ᵒ-🞰 θ↦v _ =  ↦⟨⟩ᵒ-read θ↦v ▷ ⇛ᴴ-mono λ (H‼θ≡v , θ↦v) → (-, -, 🞰⇒ H‼θ≡v) ,
    λ{ _ _ _ (-, 🞰⇒ H‼θ≡u) → -, (refl , refl) ,
      ⇛ᴴ-intro (≡∑⇒π₁≡ $ š-inj $ ◠ H‼θ≡u ◇ H‼θ≡v , θ↦v) }

  -- ← by ᵃ⟨⟩ᵒ

  ᵃ⟨⟩ᵒ-← :  θ ↦ᵒ ᵗu  ⊨ ᵃ⟨ _←ᴿ_ {T} θ v ⟩ᵒ λ _ →  θ ↦ᵒ (T , v)
  ᵃ⟨⟩ᵒ-← θ↦ _ =  ↦⟨⟩ᵒ-read θ↦ ▷ ⇛ᴴ-mono λ (H‼θ≡ , θ↦) → (-, -, ←⇒ H‼θ≡) ,
  ...
  ᵃ⟨⟩ᵒ-fau :  θ ↦ᵒ (◸ʸ Xʸ , x)  ⊨ ᵃ⟨ fauᴿ f θ ⟩ᵒ λ y →
                ⌜ y ≡ x ⌝ᵒ×  θ ↦ᵒ (-, f x)
  ᵃ⟨⟩ᵒ-fau θ↦x _ =  ↦⟨⟩ᵒ-read θ↦x ▷ ⇛ᴴ-mono λ (H‼θ≡x , θ↦x) →
    (-, -, fau⇒ H‼θ≡x) ,
    λ{ _ _ _ (-, fau⇒ H‼θ≡y) → -, (refl , refl) , ↦ᵒ-write θ↦x ▷ ⇛ᴴ-mono
  ...
  ᵃ⟨⟩ᵒ-cas-tt :  θ ↦ᵒ (◸ʸ Xʸ , x)  ⊨ ᵃ⟨ casᴿ θ x y ⟩ᵒ λ b →
                   ⌜ b ≡ tt ⌝ᵒ×  θ ↦ᵒ (-, y)
  ᵃ⟨⟩ᵒ-cas-tt θ↦x _ =  ↦⟨⟩ᵒ-read θ↦x ▷ ⇛ᴴ-mono λ (H‼θ≡x , θ↦x) →
    (-, -, cas⇒-tt H‼θ≡x) , λ _ _ _ →
    λ{ (-, cas⇒-ff H‼θ≡z z≢x) → absurd $ z≢x $ ≡∑⇒π₁≡ $ š-inj $ ◠ H‼θ≡z ◇ H‼θ≡x;
       (-, cas⇒-tt _) → -, (refl , refl) , ↦ᵒ-write θ↦x ▷ ⇛ᴴ-mono (refl ,_) }

  ᵃ⟨⟩ᵒ-cas-ff :  z ≢ x  →
    θ ↦⟨ p ⟩ᵒ (◸ʸ Xʸ , z)  ⊨ ᵃ⟨ casᴿ θ x y ⟩ᵒ λ b →
      ⌜ b ≡ ff ⌝ᵒ×  θ ↦⟨ p ⟩ᵒ (-, z)
  ᵃ⟨⟩ᵒ-cas-ff z≢x θ↦z _ =  ↦⟨⟩ᵒ-read θ↦z ▷ ⇛ᴴ-mono λ (H‼θ≡z , θ↦z) →
    (-, -, cas⇒-ff H‼θ≡z z≢x) , λ _ _ _ →
    λ{ (-, cas⇒-tt H‼θ≡x) → absurd $ z≢x $ ≡∑⇒π₁≡ $ š-inj $ ◠ H‼θ≡z ◇ H‼θ≡x;
       (-, cas⇒-ff _ _) → -, (refl , refl) , ⇛ᴴ-intro (refl , θ↦z) }

  -- alloc by ᵃ⟨⟩ᵒ

  ᵃ⟨⟩ᵒ-alloc :  ⊨ ᵃ⟨ allocᴿ n ⟩ᵒ λ θ →  θ ↦ᴸᵒ rep n ⊤-  ∗ᵒ  Freeᵒ n θ
  ᵃ⟨⟩ᵒ-alloc {n} _ =  ⇛ᴴ-intro-✓ᴴ {Pᵒ = ⊤ᵒ₀} _  ▷ ⇛ᴴ-mono λ (✓H , -) →
    (-, -, alloc⇒ _ $ ✓ᴴ-∑ň ✓H .π₁) ,
    λ{ _ _ _ (-, alloc⇒ _ Ho≡ň) → -, (refl , refl) , ↦ᴸᵒ'-alloc Ho≡ň ▷
      ⇛ᴴ-mono (∗ᵒ-mono (↦ᴸᵒ'⇒↦ᴸᵒ {ᵗvs = rep n _}) λ Free' → -, refl , Free') }
  ...
  ᵃ⟨⟩ᵒ-free {ᵗvs} lenvs≡n θ↦vs∗Free _ =  θ↦vs∗Free ▷ ∗ᵒ∃ᵒ-out ▷ λ (-, big) →
    ∗ᵒ∃ᵒ-out big ▷ λ{ (refl , big) → big ▷ ∗ᵒ-monoʳ Freeᵒ'-š ▷ ⇛ᴴ-eatˡ ▷
    ⇛ᴴ-mono (∗ᵒ∃ᵒ-out › λ (Ho≡š , big) → (-, -, free⇒ Ho≡š) ,
