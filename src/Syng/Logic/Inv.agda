...
open import Syng.Lang.Expr using (Addr; Type; V⇒E; TyVal)
open import Syng.Lang.Ktxred using (Redex; 🞰ᴿ_; Ktx; _ᴷ◁_)
open import Syng.Logic.Prop using (HorKind; Name; SProp∞; SProp˂∞; ¡ᴾ_; ⌜_⌝∧_;
  _∗_; _-∗_; _↦_; [^_]ᴺ; &ⁱ⟨_⟩_; ⅋ⁱ⟨_⟩_; static; _↦ⁱ_; Basic; ^ᶻᴺ-✔)
open import Syng.Logic.Core using (_⊢[_]_; _⊢[<_]_; Pers; Pers-⇒□; _»_; ∃-elim;
...
open import Syng.Logic.Hor using (_⊢[_][_]ᵃ⟨_⟩_; _⊢[_]⁺⟨_⟩[_]_; _⊢[<ᴾ_]⟨_⟩[_]_;
  _ᵘ»ᵃʰ_; _ᵃʰ»ᵘ_; _ᵃʰ»_; ahor-frameˡ; ahor✔-hor; hor<ᴾ-map)
open import Syng.Logic.Heap using (ahor-🞰)

-- Import and re-export
open import Syng.Logic.Judg public using (&ⁱ-⇒□; &ⁱ-resp-□∗; ⅋ⁱ-mono; ⅋ⁱ-eatˡ;
...
abstract
  ...
  ahor-↦ⁱ-🞰 :  [^ static ]ᴺ  ∗  θ ↦ⁱ (T , v)  ⊢[ ι ][ i ]ᵃ⟨ 🞰ᴿ_ {T} θ ⟩ λ u →
                 ⌜ u ≡ v ⌝∧  [^ static ]ᴺ
  ...
  hor-↦ⁱ-🞰 P⊢⟨Kv⟩Q =  ahor✔-hor {i = 0} ^ᶻᴺ-✔ (∗-assocˡ »
      ahor-frameˡ ahor-↦ⁱ-🞰 ᵃʰ» λ _ → ∃∗-elim λ u≡v → ∗-monoʳ $ ∃-intro u≡v)
