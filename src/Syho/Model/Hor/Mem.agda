...
abstract
  ...
  ᵃ⟨⟩ᵒ-fau :  θ ↦ᵒ (◸ʸ Xʸ , x)  ⊨ ᵃ⟨ fauᴿ f θ ⟩ᵒ λ y →
                ⌜ y ≡ x ⌝ᵒ×  θ ↦ᵒ (-, f x)
  ᵃ⟨⟩ᵒ-fau θ↦x _ =  ↦⟨⟩ᵒ-read θ↦x ▷ ⇛ᴹ-mono λ (M‼θ≡x , θ↦x) → (-, fau⇒ M‼θ≡x) ,
    λ{ _ _ _ (fau⇒ M‼θ≡y) → -, (refl , refl) , ↦ᵒ-write θ↦x ▷ ⇛ᴹ-mono λ θ↦fx →
  ...
  ᵃ⟨⟩ᵒ-free {ᵗvs} lenvs≡n θ↦vs∗Free _ =  θ↦vs∗Free ▷ ∗ᵒ∃ᵒ-out ▷ λ (-, big) →
    ...
    ⇛ᴹ-mono (∗ᵒ∃ᵒ-out › λ (Mo≡š , big) → (-, free⇒ Mo≡š) , λ{ _ _ _ (free⇒ _) →
