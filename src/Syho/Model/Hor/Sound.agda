...
open import Syho.Logic.Hor using (_⊢[_][_]ᵃ⟨_⟩_; _⊢[_]⁺⟨_⟩ᴾ_; _⊢[_]⁺⟨_⟩ᵀ[_]_;
  ...
  _ᵘᴺ»ⁱʰ_; _ᵃʰ»ᵘ_; _ʰ»ᵘᴺ_; ahor-frameˡ; hor-frameˡ; ahor-hor; ahor-ihor;
...
open import Syho.Model.Hor.Wp using (ᵃ⟨_⟩ᵒ; ⁺⟨_⟩ᴾᵒ; ⁺⟨_⟩ᵀᵒ; ⁺⟨_⟩∞ᵒ; ⁺⟨⟩ᴾᵒ-val;
  ...
  ⁺⟨⟩ᵀᵒ-eatˡ)
open import Syho.Model.Hor.Lang using (ᵃ⟨⟩ᵒ-⟨⟩ᴾᵒ; ᵃ⟨⟩ᵒ-⟨⟩ᵀᵒ; ᵃ⟨⟩ᵒ-⟨⟩∞ᵒ;
...
abstract
  ...
  -- ahor-ihor :  P ∗ [⊤]ᴺ  ⊢[ ∞ ][ i ]ᵃ⟨ red ⟩ (λ v →  Q˙ v ∗ [⊤]ᴺ)  →
  --              (∀ v →  Q˙ v  ⊢[ ∞ ][ j ]⟨ K ᴷ◁ V⇒E v ⟩∞)  →
  --              P  ⊢[ ∞ ][ j ]⁺⟨ ĩ₁ (-, K , red) ⟩∞

  ⊢⁺⟨⟩∞-sem (ahor-ihor P⊢⟨red⟩ᴺQ Qv⊢⟨Kv⟩∞) =  -∗ᵒ-intro (λ _ → ∗ᵒ-comm ›
    ⊢ᵃ⟨⟩-sem P⊢⟨red⟩ᴺQ › ᵃ⟨⟩ᵒ-mono λ v → ∗ᵒ-monoˡ $ ⊢⁺⟨⟩∞-sem (Qv⊢⟨Kv⟩∞ v)) ›
    ᵃ⟨⟩ᵒ-⟨⟩∞ᵒ
...
abstract
  ...
  ⊢⁺⟨⟩ᴾ-sem (ahor-hor P⊢⟨red⟩ᴺQ Qv⊢⟨Kv⟩R) =  -∗ᵒ-intro (λ _ → ∗ᵒ-comm ›
    ⊢ᵃ⟨⟩-sem P⊢⟨red⟩ᴺQ › ᵃ⟨⟩ᵒ-mono λ v → ∗ᵒ-monoˡ λ big →
    λ{ .! → big ▷ ⊢⁺⟨⟩ᴾ-sem (Qv⊢⟨Kv⟩R v .!) }) › ᵃ⟨⟩ᵒ-⟨⟩ᴾᵒ
