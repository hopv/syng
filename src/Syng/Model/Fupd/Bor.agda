...
module Syng.Model.Fupd.Bor where
...
open import Syng.Model.ERA.Bor using (Envᴮᵒʳ)
open import Syng.Model.ERA.Glob using (jᴮᵒʳ; ∅ᴵⁿᴳ)
open import Syng.Model.Prop.Base using (SPropᵒ; _⊨✓_; _⊨_; ⊨_; ⌜_⌝ᵒ×_; _∗ᵒ_;
...
open import Syng.Model.Prop.Smry using (Smry; Smry-0; Smry-add-š; Smry-rem-<;
  Smry-upd)
open import Syng.Model.Prop.Lft using ([_]ᴸ⟨_⟩ᵒ; †ᴸᵒ_; []ᴸ⟨⟩ᵒ-resp;
  []ᴸ⟨⟩ᵒ-merge-/2; []ᴸ⟨⟩ᵒ-split-/2; dup-†ᴸᵒ; []ᴸ⟨⟩ᵒ-†ᴸᵒ-no)
open import Syng.Model.Prop.Bor using (Borᵐ; &ᵐ⟨_⟩ᵒ_; Oborᵐ; ⅋ᵐ⟨_⟩ᵒ_; Lend;
  ⟨†_⟩ᵒ_; &ᵐᵒ-new'; &ᵐᵒ-make; Borᵐ-open'; Oborᵐ-close'; Lend-back')
open import Syng.Model.Prop.Basic using (⸨⸩ᴮ-Mono)
open import Syng.Model.Prop.Interp using (⸨_⸩; ⸨⸩-ᴮ⇒; ⸨⸩-Mono)
open import Syng.Model.Prop.Sound using (⊢-sem)
open import Syng.Model.Fupd.Base using ([_]⇛ᵍ¹_; ⇛ᵍ-mono✓; ⇛ᵍ-mono; ⊨✓⇒⊨-⇛ᵍ;
